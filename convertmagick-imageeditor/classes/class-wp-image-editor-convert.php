...
class WP_Image_Editor_Convert extends WP_Image_Editor {

	protected $image = null; // Imagick Object

	public function __destruct() {
		if ( $this->image instanceof ConvertMagick ) {
	...
	/**
	 * Checks to see if current environment supports ConvertMagick.
	 *
	 * @since 3.5.0
	 * @access public
	 *
	...
	public static function test( $args = array() ) {
		...
		$class_methods =  get_class_methods( 'ConvertMagick' );
		for ($i=0; $i<count($class_methods); $i++) 	$class_methods[$i] = strtolower($class_methods[$i]);
	...
	public function load() {
		...
		try {
			$this->image = new ConvertMagick( $this->file );
	...
	public function set_quality( $quality = null ) {
		...
		try {
			if ( 'image/jpeg' == $this->mime_type ) {
				...
				$this->image->setImageCompression( $image::COMPRESS_JPEG );
			}
			else {
				$this->image->setImageCompressionQuality( $quality );
			}
	...
	public function rotate( $angle ) {
		 ...
		 */
		try {
			$this->image->rotateImage( 0, 360-$angle );
